	...
	describe('max visible', ({ describe }) => {
		describe('terminal height limit', ({ test }) => {
			test('hides tasks exceeding terminal height', async () => {
				...
				expect(result.rawOutput).toMatch(/\+ \d+ queued/);
			});
			...
			test('nested subtasks count toward line limit', async () => {
				...
				expect(result.rawOutput).toContain('Child');
			});
			...
			test('multiple groups sorted together', async () => {
				...
				expect(result.rawOutput).toContain('G1-');
				expect(result.rawOutput).toContain('G2-');
			});
			...
			test('clear() shows all tasks (final render lifts limit)', async () => {
				...
				const allTasksPresent = Array.from({ length: 15 }, (_, i) => result.rawOutput.includes(`Task ${String(i + 1).padStart(2, '0')}`)).every(Boolean);
			...
			test('no extra render when tasks fit in terminal', async () => {
				...
				expect(result.rawOutput).not.toMatch(/\(\+ \d+ (queued|completed|loading)/);

				// Extract the final frame
				const lastFrame = result.rawOutput.split('\u001B[u\u001B[J').at(-1) || '';
			...
			test('terminal resize after completion preserves unlimited render', async () => {
				...
				for await (const _ of pty) {
					if (pty.rawOutput.includes('TASKS_DONE')) {
						break;
					}
				}
				await pty.resize(80, 10);
			...
			test('resize to larger terminal after completion skips unnecessary exit render', async () => {
				...
				for await (const _chunk of pty) {
					const lines = await pty.getScreen();
					if (!resized && lines.includes('ALL_DONE')) {
				...
				expect(result.screen).toBe([
					'ALL_DONE',
					...Array.from({ length: 15 }, (_, index) => `✔ Task ${String(index + 1).padStart(2, '0')}`),
				].join('\n'));

				// Extract the last two frames
				const frames = result.rawOutput.split('\u001B[u\u001B[J');
			...
			test('completed tasks expand in final output without clear()', async () => {
				...
				expect(result.screen).toBe(
					Array.from({ length: 15 }, (_, index) => `✔ Task ${String(index + 1).padStart(2, '0')}`).join('\n'),
				);
			});
		...
		describe('maxVisible option', ({ test }) => {
			...
			test('concurrent groups with different maxVisible', async () => {
				...
				expect(result.rawOutput).toContain('Small-');
				expect(result.rawOutput).toContain('Large-');
			...
			test('resets after group clears while renderer survives', async () => {
				...
				const allFreePresent = Array.from(
					{ length: 5 },
					(_, i) => result.rawOutput.includes(`Free-${i + 1}`),
				).every(Boolean);
